...
using CardiologicalResearch.Filters;
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Manage(LocalPasswordModel model)
        {
            ...
            if (hasLocalAccount)
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        ModelState.AddModelError(string.Empty, "Неправильный текущий пароль или недопустимый новый пароль.");
                    }
                }
            }
            else
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        ModelState.AddModelError(string.Empty, String.Format("Не удалось создать локальную учетную запись. Возможно, учетная запись \"{0}\" уже существует.", User.Identity.Name));
                    }
                }
            }
        ...
        public ActionResult ExternalLoginConfirmation(RegisterExternalLoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (CardioContext db = new CardioContext())
                {
